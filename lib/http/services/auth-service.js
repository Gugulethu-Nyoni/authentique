...
export default class AuthService {
  ...
  try {
    ...
    // Verify token directly — do NOT replace URL-safe base64 chars!
    const { email } = jwt.verify(decodedToken, config.jwt.secret, {
      algorithms: ['HS256']
    });
    ...
    const [result] = await db.query(
      ...
      [email, token]  // Use original token from request for DB match
    );
    ...
    if (err.name === 'JsonWebTokenError') {
      throw { status: 400, message: 'Invalid token', details: err.message };
    }
    throw { status: 400, message: 'Email confirmation failed' };
  }
}
